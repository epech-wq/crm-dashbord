  ...
  const renderWidget = (widgetKey: string, widget: any) => {
    ...
    switch (widgetKey) {
      ...
      case "charts":
        return (
          <div key={widgetKey} className={sizeClasses[widget.size]}>
            <ChartsGrid period={filters.period} visibleCharts={dashboardLayout.visibleCharts} />
          </div>
        )
      case "orders":
        return (
          <div key={widgetKey} className="col-span-full">
            <Card>
              <CardHeader>
                <CardTitle className="text-xl font-bold">Pedidos Filtrados ({filteredOrders.length})</CardTitle>
                <CardDescription className="text-base">
                  Pedidos que coinciden con los filtros aplicados •
                  Total: ${filteredOrders.reduce((sum, order) => sum + order.amount, 0).toLocaleString()} •
                  Margen promedio: {filteredOrders.length > 0 ? (filteredOrders.reduce((sum, order) => sum + order.margin, 0) / filteredOrders.length).toFixed(1) : 0}%
                </CardDescription>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 p-4 bg-muted/30 rounded-lg">
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-bold text-red-600">
                      {filteredOrders.filter(o => o.status === "Cancelado").length}
                    </div>
      ...
      case "map":
        return (
          <div key={widgetKey} className={sizeClasses[widget.size]}>
            <MapSystemEnhanced orders={filteredOrders} />
          </div>
        )
      default:
        return null
      ...
      <header className="sticky top-0 z-50 border-b bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/60 shadow-sm">
        <div className="flex h-18 items-center justify-between px-8">
          <div className="flex items-center space-x-4">
            ...
            <Badge variant="secondary" className="text-sm font-medium px-3 py-1">Analytics</Badge>
          </div>
              ...
              <Sheet open={isQuickControlOpen} onOpenChange={setIsQuickControlOpen}>
                ...
                <SheetContent side="right" className="w-96">
                  <SheetHeader>
                    ...
                    <SheetDescription>
                      {dashboardLayout.description} • Mostrando {filteredOrders.length} de {mockOrders.length} pedidos
                    </SheetDescription>
                  </SheetHeader>
